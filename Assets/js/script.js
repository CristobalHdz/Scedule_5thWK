...
$(document).ready(function () {
    ...
    var lockDisable = false; // Local storage buttons are locked when false
    // Get current Time 
    var dt = new Date();
    var hh = dt.getHours();
    ...
    // What will excecute when the program starts
    colorChange();
    changeButtonLock();

    // Unlocks and locks save buttons
    $('#unlockBtn').on("click",function () {
        if (lockDisable === false) {
            lockDisable = true;
            $('.saveBtn').removeAttr('disabled', 'true'); //enables button
            $('.saveBtn').removeClass('saveBtnDisabled');
            $('.saveBtn').addClass('saveBtnEnabled');
            $('.saveBtn').children().attr('src', "./Assets/SaveImg.png"); //Save image
        } else {
            lockDisable=false
            changeButtonLock();
        };
